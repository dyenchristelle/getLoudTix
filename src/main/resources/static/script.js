...
//retrieve, check if email already exists (must be not yet existing)
      try {
        const response = await fetch(`https://a189-136-158-66-185.ngrok-free.app/api/checkReservation?email=${encodeURIComponent(email)}`);
...
//retrieve, check if email exists (must be existing)
      try {
        ...
        if (data.exists) {
          ...
          const deleteResponse = await fetch(`https://a189-136-158-66-185.ngrok-free.app/api/deleteReservation?email=${encodeURIComponent(email)}`,{
              method: "DELETE",
            }
          );
...
`;
  ...
  function updateSlotDisplay() {
    fetch('https://a189-136-158-66-185.ngrok-free.app/api/slots') // will fetch the avail_slot table
  ...
  const initApp = () => {
    ...
    fetch("https://a189-136-158-66-185.ngrok-free.app/tickets.json")
      .then((response) => {
        if (!response.ok) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function submitReservation(formData) {
    ...
    fetch("https://a189-136-158-66-185.ngrok-free.app/api/submitChoice", {
      method: "POST",
